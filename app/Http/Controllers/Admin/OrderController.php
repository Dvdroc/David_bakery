...
class OrderController extends Controller
{
    ...
    public function store(Request $request){
        // 1. Validasi - Hapus 'payment_proof' dari daftar required
        $request->validate([
        ...
        // 3. Cek Penutupan Tanggal & Kuota (Tetap sama)
        $isClosed = ProductClosure::where('product_id', $product->id)
                    ->where('date', $pickupDate)
                    ->exists();

        if ($isClosed) {
            return back()->with('error', 'Produk ini tidak tersedia pada tanggal tersebut.');
        }
        ...
        if ($requestedQuantity > ($product->daily_quota - $sold)) {
            return back()->with('error', 'Kuota tidak cukup.');
        }

        // 4. Simpan Order - Status pembayaran diubah menjadi 'unpaid' atau langsung 'pending'
        $order = Order::create([
            'user_id' => Auth::id(),
            ...
            'status' => 'pending', 
            'payment_status' => 'pending', // Ubah dari pending_dp
            'total_price' => $grandTotal,
            'dp_amount' => 0,              // Set 0 karena tidak ada DP
            'payment_proof' => null,       // Set null
        ]);
        ...
        return redirect()->route('user.list-pesanan')->with('success', 'Pesanan berhasil dibuat!');
    }
    ...
    public function submitReview(Request $request, $id){
        ...
        // Review hanya bisa diberikan jika pesanan sudah completed
        if ($order->status !== 'completed') {
            return back()->with('error', 'Anda hanya bisa memberi ulasan pada pesanan yang sudah selesai.');
        }
