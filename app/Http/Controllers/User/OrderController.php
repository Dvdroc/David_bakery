...
class OrderController extends Controller
{
    ...
    public function show($id, Request $request)
    {
        ...
        // Hitung rata-rata dan total ulasan
        $avgRating = $product->reviews()->avg('rating') ?: 0;
        $totalReviews = $product->reviews()->count();
        
        // Hitung distribusi bintang (untuk progress bar)
        $starCount = $product->reviews()->where('rating', 5)->count();
        $percentage5 = $totalReviews > 0 ? ($starCount / $totalReviews) * 100 : 0;

        // ... (kode penentuan slot tanggal tetap sama) ...
        $today = Carbon::today();
        ...
        for ($date = $minDate->copy(); $date->lte($maxDate); $date->addDay()) {
            ...
            $isClosed = ProductClosure::where('product_id', $product->id)->where('date', $dateStr)->exists();
        ...
        return view('user.order', compact(
            'product', 'slots', 'minDate', 'maxDate', 
            'avgRating', 'totalReviews', 'percentage5'
        ));
    }
